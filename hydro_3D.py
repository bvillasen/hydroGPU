...
def solvePoisson():
  ...
  for n in range(maxIter):
    ...
    if converged.get()[0] == 1: 
      print 'Poisson converged: ', n+1 
      return   
  print 'Poisson converged: ', maxIter 
########################################################################
def getGravForce():
  solvePoisson()
  tex_1.set_array( phi_array_1 )
...
#reduceDensity_kernel( np.int32(nWidth),  np.float32(nHeight), np.float32(nDepth), np.float32(dx), np.float32(dy), np.float32(dz),
		   #np.float32(xMin), np.float32(yMin), np.float32(zMin), cnsv1_d, rhoReduced_d, 
		   #blockX_d, blockY_d, blockZ_d, grid=grid3D, block=block3D )



#getBounderyPotential_kernel( np.float32(pi4), np.int32(nBlocksGrid), np.int32(nWidth),  np.float32(nHeight), np.float32(nDepth), np.float32(dx), np.float32(dy), np.float32(dz), 
			    #np.float32(xMin), np.float32(yMin), np.float32(zMin), rhoReduced_d, phiWall_l_d, 
			    #blockX_d, blockY_d, blockZ_d, grid=grid2D, block=block2D )
...
plt.figure( 1 )
phi =  phi_d.get() 
plt.imshow( phi[:,:,nWidth/2], extent=[xMin, xMax, yMin, yMax] )
plt.colorbar()
#plt.show()
...
plt.imshow( force[:,:,nWidth/2], extent=[xMin, xMax, yMin, yMax] )
plt.colorbar()
...
#plt.figure( 5 )
#phiWall =  phiWall_l_d.get() 
#plt.imshow( phiWall, extent=[xMin, xMax, yMin, yMax] )
#plt.colorbar()
