...
__global__ void getInterFlux_hll( const int coord, const cudaP dt,  const cudaP gamma, const cudaP dx, const cudaP dy, const cudaP dz, 
  ...
  cnsv_2[ tid ] = cnsv_2[ tid ] - delta*( iFlx2_r - iFlx2_l ) + dt*gForceX[tid];
  cnsv_3[ tid ] = cnsv_3[ tid ] - delta*( iFlx3_r - iFlx3_l ) + dt*gForceY[tid];
  cnsv_4[ tid ] = cnsv_4[ tid ] - delta*( iFlx4_r - iFlx4_l ) + dt*gForceZ[tid];
  cnsv_5[ tid ] = cnsv_5[ tid ] - delta*( iFlx5_r - iFlx5_l ) + dt*gravWork[tid];
}  
...
__global__ void iterPoissonStep( int* converged, const int paridad, 
  ...
  //Boundary conditions
//   if  ( t_j == 0 )        phi_l = cudaP( phiWall[ t_i + t_k*nWidth ] );
//   if  ( t_j == nWidth-1 ) phi_r = cudaP( phiWall[ t_i + t_k*nWidth ] );
//   if  ( t_i == 0 )        phi_d = cudaP( phiWall[ t_k + t_j*nWidth ] );
//   if  ( t_i == nWidth-1 ) phi_u = cudaP( phiWall[ t_k + t_j*nWidth ] );
//   if  ( t_k == 0 )        phi_b = cudaP( phiWall[ t_j + t_i*nWidth ] );
//   if  ( t_k == nWidth-1 ) phi_t = cudaP( phiWall[ t_j + t_i*nWidth ] );
...
//   if ( ( blockIdx.x == 0 ) ||  ( blo == nWidth-1 ) || ( t_i == 0 ) || ( t_i == nWidth-1 ) || ( t_k == 0 ) || ( t_k == nWidth-1 ) ) return;
  ...
  if ( ( abs( ( phi_new - phi_c ) / phi_c ) > 0.003 ) ) converged[0] = 0;
